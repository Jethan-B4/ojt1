      ...
      {MAIN_TABS.map((tab) => {
        ...
        return (
          <TouchableOpacity
            ...
            <Text
              className={`text-[13px] font-semibold ${on ? "text-white" : "text-gray-400"}`}>
              {tab.short}
            </Text>
          </TouchableOpacity>
        );
...
export default function ProcurementScreen({ route }: { route?: any }) {
  const [active, setActive] = useState<MainTab>("purchase_request");

  // If returning from canvassing with activeSubTab param, pass it to PRModule
  const activeSubTab = route?.params?.activeSubTab ?? undefined;

  return (
    <View className="flex-1 bg-gray-50">
      <TabStrip active={active} onSelect={setActive} />
      {active === "purchase_request" && (
        <PRModule initialSubTab={activeSubTab} />
      )}
