 ...
 */
const STATUS_ID_CFG: Record<number, { bg: string; text: string; dot: string; label: string }> = {
...
const STAGE_CFG = [
  { stage: 1, label: "Request & Approval",   shortLabel: "Phase 1", color: "#3b82f6" },
  { stage: 2, label: "Canvass & Evaluation", shortLabel: "Phase 2", color: "#f59e0b" },
  { stage: 3, label: "Order & Delivery",     shortLabel: "Phase 3", color: "#10b981" },
  { stage: 4, label: "Payment & Closure",    shortLabel: "Phase 4", color: "#8b5cf6" },
];
...
function useAdminData() {
  ...
  const inProgress = prs.filter((p) => p.statusId >= 2 && p.statusId <= 4).length;
  ...
  const thisWeek = (() => {
    ...
    return rows.filter((r) => r.created_at && new Date(r.created_at) >= ago).length;
  })();

  const statCards: StatCard[] = [
    { label: "Total PRs",        value: total,      sub: `+${thisWeek} this week`,    subType: "up",      icon: "description"     },
    { label: "Pending",          value: pending,    sub: "Awaiting Div. Head review", subType: "warn",    icon: "pending-actions" },
    { label: "In Approval Flow", value: inProgress, sub: "Div. Head → BAC → Budget",  subType: "info",    icon: "autorenew"       },
    { label: "Awaiting PARPO",   value: atParpo,    sub: "Final approval step",       subType: "warn",    icon: "verified-user"   },
    { label: "Approved",         value: approved,   sub: "Phase 1 complete",          subType: "ok",      icon: "check-circle"    },
    { label: "High-Value PRs",   value: highValue,  sub: "Total cost > ₱10,000",      subType: "neutral", icon: "star"            },
  ];
  ...
  const statusBreakdown = [1, 2, 3, 4, 5, 6]
    .map((sid) => ({
      id:    sid,
      label: statuses.find((s) => s.id === sid)?.status_name ?? statusCfgFor(sid).label,
  ...
  return {
    prs, recent, statCards, stages, statusBreakdown,
    loading, error, refresh: load, lastRefresh,
  };
}
...
function useProcessorData(roleId: number) {
  ...
  const statCards: StatCard[] = [
    {
      label: "Action Required", value: queue.length,
      sub:   queue.length > 0 ? "PRs awaiting your review" : "Queue is clear",
      subType: queue.length > 0 ? "warn" : "ok",
      icon: "pending-actions",
    },
    { label: "Total System PRs", value: prs.length,       sub: "All divisions",          subType: "info", icon: "description"     },
    { label: "Forwarded",        value: forwarded.length, sub: "Past your approval step", subType: "up",   icon: "forward"         },
    { label: "Fully Approved",   value: approved,         sub: "Phase 1 complete",        subType: "ok",   icon: "check-circle"    },
  ];
  ...
  return {
    queue, recentOther, statCards, totalCount: prs.length,
    loading, error, refresh: load, lastRefresh, queueStatusId,
  };
}
...
function useEndUserData(divisionId: number | null) {
  ...
  const prs      = rows.map((r) => rowToSummary(r, statuses));
  const recent   = [...prs].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 5);
  const pending  = prs.filter((p) => p.statusId === 1).length;
  const inFlow   = prs.filter((p) => p.statusId >= 2 && p.statusId <= 4).length;
  const approved = prs.filter((p) => p.statusId === 6).length;

  const statCards: StatCard[] = [
    { label: "My PRs",           value: prs.length, sub: "Total submitted",           subType: "info", icon: "description"     },
    { label: "Pending",          value: pending,    sub: "Awaiting Div. Head review",  subType: "warn", icon: "pending-actions" },
    { label: "In Approval Flow", value: inFlow,     sub: "Being reviewed",             subType: "up",   icon: "autorenew"       },
    { label: "Approved",         value: approved,   sub: "Phase 1 complete",           subType: "ok",   icon: "check-circle"    },
  ];

  return {
    prs, recent, statCards,
    loading, error, refresh: load, lastRefresh,
  };
}
...
function ErrorBanner({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <View style={{ margin: 16, backgroundColor: "#fef2f2", borderRadius: 12, borderWidth: 1,
      borderColor: "#fecaca", padding: 14, flexDirection: "row", alignItems: "center", gap: 10 }}>
      <MaterialIcons name="error-outline" size={20} color="#dc2626" />
      <View style={{ flex: 1 }}>
        <Text style={{ fontSize: 12.5, fontWeight: "700", color: "#991b1b" }}>Failed to load data</Text>
        <Text style={{ fontSize: 11.5, color: "#b91c1c", marginTop: 2 }}>{message}</Text>
      </View>
      <TouchableOpacity onPress={onRetry} activeOpacity={0.8}
        ...
        <Text style={{ fontSize: 11, fontWeight: "700", color: "#ffffff" }}>Retry</Text>
      </TouchableOpacity>
    </View>
  );
}
...
function LastRefreshedBadge({ time }: { time: Date | null }) {
  if (!time) return null;
  const label = time.toLocaleTimeString("en-PH", { hour: "2-digit", minute: "2-digit" });
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 4,
      alignSelf: "flex-end", paddingHorizontal: 16, paddingTop: 6, paddingBottom: 2 }}>
  ...
  return (
    <View style={{
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between",
        ...
        <View style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: "#f0fdf4",
        ...
        <Text style={{ fontSize: 10, color: "#9ca3af", fontWeight: "600",
          ...
          maxWidth: "60%" }} numberOfLines={2}>
          {card.label}
        </Text>
      </View>
      <Text style={{ fontSize: 32, fontWeight: "800", color: CLR.brand900, lineHeight: 36, marginBottom: 6 }}>
        {card.value}
      </Text>
      <View style={{ backgroundColor: c.bg, paddingHorizontal: 8, paddingVertical: 3,
...
function SectionHeader({
  ...
  return (
    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center",
      ...
      <View style={{ flex: 1 }}>
        <Text style={{ fontSize: 14, fontWeight: "800", color: "#111827", letterSpacing: -0.3 }}>
          {title}
        </Text>
        {sub && (
      ...
      {onViewAll && (
        <TouchableOpacity onPress={onViewAll} hitSlop={8}>
          <Text style={{ fontSize: 12, fontWeight: "600", color: CLR.brand700 }}>View all →</Text>
        </TouchableOpacity>
      )}
    </View>
  );
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.7}
      ...
      <View style={{ flex: 1, gap: 2 }}>
        <Text style={{ fontSize: 11.5, fontWeight: "700", color: CLR.brand900, fontFamily: MONO }}>
          {record.prNo}
        </Text>
      ...
      <View style={{ width: 68, alignItems: "center" }}>
        <Text style={{
          fontSize: 9.5, fontWeight: "700", color: CLR.brand700,
      ...
      <Text style={{
        ...
        fontFamily: MONO, width: 60, textAlign: "right",
      }}>
...
function PRTrackerCard({ pr, onPress }: { pr: PRSummary; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85}
      ...
      <View style={{
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 12, fontWeight: "700", color: CLR.brand900, fontFamily: MONO }}>
            {pr.prNo}
          </Text>
          <Text style={{ fontSize: 11.5, color: "#6b7280", marginTop: 2 }} numberOfLines={1}>
            {pr.purpose}
          </Text>
        </View>
        <View style={{
          flexDirection: "row", alignItems: "center", gap: 4,
          backgroundColor: cfg.bg, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999,
        }}>
          <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: cfg.dot }} />
          <Text style={{ fontSize: 10, fontWeight: "700", color: cfg.text }}>{cfg.label}</Text>
        </View>
              ...
              <React.Fragment key={step.key}>
                <View style={{ alignItems: "center", gap: 4 }}>
                  <View style={{
                    ...
                    backgroundColor: done ? CLR.brand900 : active ? "#ecfdf5" : "#f9fafb",
                    borderWidth: active ? 2 : 1,
                    borderColor: active ? CLR.brand500 : done ? CLR.brand900 : "#e5e7eb",
                    alignItems: "center", justifyContent: "center",
                  }}>
                    <MaterialIcons
                      name={done ? "check" : step.icon} size={15}
                      color={done ? "#ffffff" : active ? CLR.brand500 : "#d1d5db"}
                    />
                  </View>
                  <Text style={{
                    fontSize: 8.5,
                    fontWeight: active || done ? "700" : "500",
                    color: done ? CLR.brand900 : active ? CLR.brand500 : "#9ca3af",
                    textAlign: "center", maxWidth: 38,
                  }} numberOfLines={1}>
                    {step.label}
                  </Text>
                </View>
                {i < PR_STEPS.length - 1 && (
                  <View style={{
                    flex: 1, height: 2, marginBottom: 14, marginHorizontal: 2,
                    backgroundColor: i < currentStep ? CLR.brand900 : "#e5e7eb",
                  }} />
                )}
              </React.Fragment>
            );
      ...
      <View style={{
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          {pr.isHighValue && (
            <View style={{ backgroundColor: "#022c22", paddingHorizontal: 6,
              paddingVertical: 2, borderRadius: 6 }}>
              <Text style={{ fontSize: 9, fontWeight: "700", color: "#a7f3d0" }}>HIGH-VALUE</Text>
            </View>
          )}
  ...
  return (
    <View style={{ paddingHorizontal: 16, paddingBottom: 4, gap: 10 }}>
      <View style={{
        ...
        {stages.map((st) => (
          <View key={st.stage}
            style={{ flex: Math.max(st.count / total, 0.01), backgroundColor: st.color }} />
        ))}
      </View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        ...
        {stages.map((st) => (
          <View key={st.stage} style={{
            backgroundColor: st.color + "15", borderRadius: 10,
      ...
      {breakdown.length === 0 ? (
        ...
        breakdown.map((b) => (
          <View key={b.id}>
            ...
            <View style={{ height: 7, borderRadius: 999, backgroundColor: "#f3f4f6" }}>
              <View style={{
                height: 7, borderRadius: 999, backgroundColor: b.color,
                width: `${Math.max(Math.round((b.count / total) * 100), 2)}%` as any,
              }} />
            </View>
          </View>
        ))
        ...
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 4 }}>
            <View style={{
              ...
              <Text style={{
                fontSize: 10, fontWeight: "700", color: CLR.brand100,
...
export default function DashboardScreen({ navigation }: any) {
  ...
  if (roleId in ROLE_QUEUE_STATUS) return <ProcessorDashboard navigation={navigation} roleId={roleId} />;
...
function AdminDashboard({ navigation }: any) {
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={CLR.brand500} />
      }>
      ...
      {/* ── KPI stat cards ── */}
      <View style={{ paddingHorizontal: 12, paddingTop: 8, flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
        {statCards.map((card) => <StatPill key={card.label} card={card} />)}
      </View>
      ...
      {/* ── Approval funnel ── */}
      <View style={{
        marginHorizontal: 12, marginTop: 16, backgroundColor: "#ffffff", borderRadius: 16,
      ...
      {/* ── Recent PRs table ── */}
      <View style={{
        marginHorizontal: 12, marginTop: 12, backgroundColor: "#ffffff", borderRadius: 16,
        ...
        {/* Column headers */}
        <View style={{
          flexDirection: "row", paddingHorizontal: 14, paddingBottom: 8,
          borderBottomWidth: 1, borderBottomColor: "#f3f4f6",
        }}>
          {[
        ...
        {recent.length === 0 ? (
          <View style={{ paddingVertical: 28, alignItems: "center", gap: 8 }}>
            ...
            <Text style={{ fontSize: 13, color: "#9ca3af" }}>No purchase requests found.</Text>
          </View>
        ) : (
          recent.map((record, i) => (
            <PRTableRow
              key={record.id} record={record} isEven={i % 2 === 0}
...
function ProcessorDashboard({ navigation, roleId }: { navigation: any; roleId: number }) {
  const { currentUser } = useAuth();
  const {
    queue, recentOther, statCards,
    loading, error, refresh, lastRefresh, queueStatusId,
  ...
  if (loading && queue.length === 0 && recentOther.length === 0) return <LoadingScreen />;
    ...
    <ScrollView
      ...
      <View style={{ marginTop: 20, paddingHorizontal: 16, paddingBottom: 10 }}>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {queue.length > 0 && (
            <View style={{
              backgroundColor: queueCfg.bg, paddingHorizontal: 10, paddingVertical: 4,
              borderRadius: 999, borderWidth: 1, borderColor: queueCfg.dot + "60",
            }}>
              <Text style={{ fontSize: 12, fontWeight: "800", color: queueCfg.text }}>
      ...
      {queue.length === 0 ? (
        <View style={{
          marginHorizontal: 12, backgroundColor: "#f0fdf4", borderRadius: 16,
          borderWidth: 1, borderColor: "#bbf7d0", padding: 28,
          alignItems: "center", gap: 8,
        }}>
          <MaterialIcons name="check-circle" size={40} color={CLR.brand500} />
          <Text style={{ fontSize: 14, fontWeight: "700", color: CLR.brand900 }}>
            Queue is clear!
          </Text>
        ...
        <View style={{
          marginHorizontal: 12, backgroundColor: "#ffffff", borderRadius: 16,
          borderWidth: 1, borderColor: "#e5e7eb", overflow: "hidden",
          shadowColor: "#000", shadowOffset: { width: 0, height: 1 },
          shadowOpacity: 0.05, shadowRadius: 6, elevation: 2,
        }}>
  ...
  return (
    <ScrollView
      ...
      <SectionHeader
        ...
        sub={prs.length > 0 ? `${prs.length} total in your division` : undefined}
        onViewAll={prs.length > 0 ? () => navigation?.navigate?.("Procurement") : undefined}
      />

      {recent.length === 0 ? (
        <View style={{
          marginHorizontal: 12, backgroundColor: "#f9fafb", borderRadius: 16,
          borderWidth: 1, borderColor: "#e5e7eb", padding: 28,
...
function QuickActionGrid({ navigation, roleId }: { navigation: any; roleId: number }) {
