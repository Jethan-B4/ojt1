 ...
 *   • Delivery remarks
 *
 * Submission tags the remark with [DELIVERY].
 */
...
export function DeliveryRemarkSheet({
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      let finalRemark = `[DELIVERY] ${remarksText.trim()}`;
      if (fileUri && fileName) {
        const remote = buildRemotePath(`DEL-${record.deliveryNo}`, fileName);
              ...
              ListHeaderComponent={
                <View className="bg-white mx-4 mb-6 rounded-2xl border border-gray-200 p-4 shadow-sm">
                  ...
                  <TouchableOpacity
                    ...
                    className={`flex-row items-center justify-center gap-2 py-3 rounded-xl ${remarksText.trim() && !saving ? "bg-[#064E3B]" : "bg-gray-200"}`}
                  >
                    {saving ? (
